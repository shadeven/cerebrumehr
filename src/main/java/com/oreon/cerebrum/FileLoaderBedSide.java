...
public class FileLoaderBedSide {
	//"C:\md\\"
	public static final String dirPath = "C:\\md\\bedsidedx\\II - Heart, Vascular";
	...
	public static void printChildren(String filename) throws Exception {
		...
		Element title = doc.getElementsByClass("TLV1").get(1);
		
		//System.out.println(title.html());
		
		Elements titles = title.getElementsByClass("HD");
		
		Elements diffs = title.getElementsByClass("LS");
		
		for (Element diff : diffs) {
			Elements children = diff.getAllElements();
			for (Element element : children) {
				System.out.println(element.getAllElements().first().ownText());
			}
		...
		for (Element element : titles) {
			...
			if(!element.ownText().contains("Differential Overview"))
				System.out.println("------" + element.ownText() + "------");
			//Elements diffs = title.getElementsByClass("LS");
